...
4. Extract the `.zip` file to find `piantor_pro_bt-piantor_pro_bt-zmk.uf2`
...
2. **Mount the device** (if not auto-mounted)
   ```bash
   # Check if mounted
   lsblk
...
3. **Flash the firmware**
   ...
   cp piantor_pro_bt-piantor_pro_bt-zmk.uf2 /run/media/$USER/KEEBART/

   # The keyboard will automatically reboot after copying
   ```

4. **Flash both halves**
   - Repeat the above steps for the other half of the split keyboard
   - Both halves need to be flashed with the same firmware
